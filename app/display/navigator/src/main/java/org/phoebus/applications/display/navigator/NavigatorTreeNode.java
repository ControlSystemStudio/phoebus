    ...
    private static Consumer<Target> createLoadAction_OPI(String absolutePath,
        ...
        Consumer<Target> loadAction = target -> {
            ...
            if (target == Target.CurrentTab && activeDockItem.getApplication() instanceof DisplayRuntimeInstance) {
                ...
                if (!navigatorController.editModeEnabledProperty.get()) {
                    navigatorController.treeView.refresh(); // Ensure that the background color of each cell is set correctly. In edit mode, this interferes with the drag'n'drop functionality, and hence no refresh of the treeview is made.
                }
            }
            else {
