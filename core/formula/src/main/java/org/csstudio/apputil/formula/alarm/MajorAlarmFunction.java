...
/**
 * Returns a major alarm when the given condition evaluates as true.
 *
 */
public class MajorAlarmFunction implements FormulaFunction
{
    ...
    {
        return "Returns a string with major severity when the given condition is true.";
    }
    ...
    public VType compute(VType... args) throws Exception {
        ...
        if(args[1] instanceof VString)
        {
            ...
            {
                return VString.of("True: " + message.getValue(), Alarm.of(AlarmSeverity.MAJOR, AlarmStatus.CLIENT, message.getValue()), Time.now());
            } else
            {
